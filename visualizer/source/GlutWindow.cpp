#include "DataTypes.h"
#include "Window.h"
#include "GlutKeyBindings.h"
#include "GlutHeaders.h"

using namespace dg;

static UInt gs_uiGLUTModifiers = 0;
//----------------------------------------------------------------------------
static void ReshapeCallback(int iWidth, int iHeight)
{
	Window* pkWindow = Window::current();
	if ( pkWindow )
...
//----------------------------------------------------------------------------
static void KeyDownCallback(UChar ucKey, Int iX, Int iY)
{
	Window* pkWindow = Window::current();
	if ( pkWindow )
...
//----------------------------------------------------------------------------
void Window::onUpdate( Int iValue )
{
	postRedisplay();
...
//----------------------------------------------------------------------------
void Window::drawString(const Char* pcString, Int iX, Int iY, const Color& rkColor)
{
	UInt uiLen, i;
...
void Window::drawImage(
	const UChar* aucBuffer, UInt uiPosX, UInt uiPosY,
	UInt uiWidth, UInt uiHeight, bool bAlpha)
{
    if ( !aucBuffer )
        return;
...
//----------------------------------------------------------------------------
void Window::drawImage(
	const Real* afBuffer, UInt uiPosX, UInt uiPosY,
	UInt uiWidth, UInt uiHeight, bool bAlpha)
{
    if ( !afBuffer )
        return;
